...
$bookings = $db->query("SELECT b.*, u.name as user_name, s.name as stadium_name
                        FROM bookings b
                        JOIN users u ON b.user_id = u.id
                        JOIN stadiums s ON b.stadium_id = s.id
                        ORDER BY b.created_at DESC")->fetchAll();

$pageTitle = $lang['menu_bookings'] ?? 'كافة الحجوزات - المسؤول';
    ...
    <main class="admin-main">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 2.5rem; flex-wrap: wrap; gap: 1.5rem;">
            <div style="display: flex; align-items: center; gap: 1.5rem;">
                ...
                <div>
                    <h1 style="font-size: 2.5rem; font-weight: 900; letter-spacing: -0.02em;"><?= $lang['menu_bookings'] ?? 'كافة الحجوزات' ?></h1>
                    <p style="color: var(--c-text-muted); font-weight: 600;"><?= $lang['admin_bookings_desc'] ?? 'سجل كامل لجميع عمليات الحجز في النظام' ?></p>
                </div>
            </div>
            <div style="background: var(--c-surface); padding: 0.75rem 1.5rem; border-radius: 16px; border: 1px solid var(--c-border); font-weight: 800; color: var(--c-primary);"><?= count($bookings) ?> <?= $lang['stat_bookings'] ?? 'حجز فعال' ?></div>
        </div>
            ...
            <div class="table-container">
                <table class="pro-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><span style="font-weight: 800; color: var(--c-primary);">#<?= $booking['id'] ?></span></td>
                            <td style="font-weight: 700;"><?= htmlspecialchars($booking['user_name']) ?></td>
                            <td><span style="font-weight: 800;"><?= htmlspecialchars($booking['stadium_name']) ?></span></td>
                            <td style="color: var(--c-text-muted); font-weight: 600;"><?= date('Y/m/d', strtotime($booking['booking_date'])) ?></td>
                            <td style="font-weight: 700;"><?= substr($booking['start_time'], 0, 5) . ' - ' . substr($booking['end_time'], 0, 5) ?></td>
                            <td>
                                <span class="badge badge-<?= $booking['status'] ?>">
                                    <?= $booking['status'] == 'pending' ? ($lang['status_pending'] ?? 'قيد الانتظار') : ($booking['status'] == 'confirmed' ? ($lang['status_confirmed'] ?? 'مؤكد') : ($lang['status_cancelled'] ?? 'ملغى')) ?>
                                </span>
                            </td>
                        </tr>
                        <?php endforeach; ?>
