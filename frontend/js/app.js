...
const app = {
    ...
    loadTranslations: async () => {
        ...
        try {
            const [arRes, enRes] = await Promise.all([
                fetch(root + 'locales/ar.json'),
                fetch(root + 'locales/en.json')
            ]);
    ...
    toggleLanguage: () => {
        ...
        if (document.body) document.body.classList.toggle('lang-en', state.lang === 'en');
        app.translatePage();
        app.broadcastAuthState();
    },
    ...
    handleBookingClick: (stadiumId) => {
        ...
        location.href = (window.root || './') + 'booking.php?id=' + stadiumId;
    },
